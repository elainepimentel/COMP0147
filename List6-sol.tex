...
\item \streasy (Rosen 2.2-16) Let $A$ and $B$ be sets. Show that:
\begin{enumerate}
...
\item  $A_i=(0,i)$, i.e., the set $\{x \in \mathbb{R} \mid 0<x<i\}$.
\solution{
\begin{align*}
\bigcup_{i=1}^{\infty} (0,i) &= (0,1) \cup (0,2) \cup (0,3) \cup \ldots = (0,\infty) \\
\bigcap_{i=1}^{\infty} (0,i) &= (0,1) \cap (0,2) \cap (0,3) \cap \ldots = (0,1)
\end{align*}
}

\item  $A_i=(i,\infty)$, i.e., the set $\{x \in \mathbb{R} \mid x > i \}$.
\solution{
\begin{align*}
...
\item \streasy (Rosen 2.3-4) Find the domain and range 
...
\item  the function that assigns the next largest integer
...
\solution{
Function: $\{0,1\}^{*} \to \mathbb{N}$, where $\{0,1\}^\ast$ denotes the set of all
finite binary strings.  
Here the operator ${}^\ast$ is the \emph{Kleene star}, which means ``all finite strings over the given alphabet.''
}

\item The function that assigns to each binary string its length (total number of bits).
\solution{
Function: $\{0,1\}^\ast \to \mathbb{N}$, where $\{0,1\}^\ast$ denotes the set of all finite binary strings (Kleene star).
}
...
\solution{
Let $x,y \in A$ be such that $(f\circ g)(x) = (f\circ g)(y)$.
From the definition of function composition, it follows $f(g(x)) = f(g(y))$.
Recall that in $f \circ g$, the rightmost function $g$ acts first, and then $f$ is applied to the result.
Since $f$ is injective, this implies $g(x) = g(y)$.
Since $g$ is also injective, it follows that $x = y$.
...
\solution{
Recall that in $f \circ g$, the rightmost function $g$ acts first, and then $f$ is applied to the result.
To prove that $f \circ g : A \to C$ is surjective, we must show that for every $z \in C$, there is some $x \in A$ such that $(f \circ g)(x) = z$.
Let $z$ be any element in $C$.
Because $f$ is surjective, there is some $y \in B$ with $f(y) = z$, and because $g$ is surjective, there is some $x \in A$ with $g(x) = y$.
Therefore, $(f \circ g)(x) = f(g(x)) = f(y) = z$.
Since $z$ was an arbitrary element of $C$, this shows that $f \circ g$ is surjective.
}
\end{enumerate}
