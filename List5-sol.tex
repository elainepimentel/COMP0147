...
\item (Rosen 4.3-27) Let $S$ be a subset of ordered pairs 
...
\solution{Let $s$ be a string of decimal digits. Denote by $xs'$ a string with first digit $x$ and remaining string $s'$.
\[
m(s) =
\begin{cases}
d, & \text{if } s \text{ has only one digit } d, \\[6pt]
\min(x,\, m(s')), & \text{if } s = xs'.
\end{cases}
\]
...
\ie, the string formed by appending $y$ to the end of $x$.
