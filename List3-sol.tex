...
If the first professor did not want coffee, she would know that
...
because if she did not, she would have answered \qm{no} to the waitress's question.
...
concludes that she is the only one who does not want coffee.
}
...
OBS. There are many other equivalent ways of solving this question. 
\end{enumerate}
}
...
\solution{
Assume the universe of discourse is the set of all red and blue dots.
Let $P(x)$ be the proposition \prop{$x$ is a red dot}, and $Q(x)$ the proposition \prop{$x$ is a blue dot}.
Clearly, saying that every dot is blue or red 
(i.e., $\forall x. (P(x) \vee Q(x))$) is not the same as saying that all dots 
are blue or all dots are red (i.e., $\forall x. P(x) \vee \forall x. Q(x)$).
}
...
\solution{$\exists y. \forall x. ( L(x,y) \wedge (\forall z. L(x,z) \imp x=z))$. Observe that it would be the same to affirm 
$\exists y. \forall x. (L(x,y) \wedge \neg(\exists z. L(x,z) \wedge x\not=z))$.}
